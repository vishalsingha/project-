...
def register(request):
    global OTP ,first_name, last_name, username, email, password1
    if request.method =='POST':
        ...
        if password1 == password2:
            if User.objects.filter(username=username).exists():
                ...
                email_body = "Thanks for visiting farm app. OTP for your account is {}".format(OTP)
...
def check_otp(request):
    if request.method == 'POST':
        ...
        if OTP==int(otp):
            ...
            messages.info(request,"OTP didn't match")
            return redirect('/accounts/register')
